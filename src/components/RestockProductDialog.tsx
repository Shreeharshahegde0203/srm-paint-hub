...
const RestockProductDialog = ({ product, onRestock, onClose }: RestockProductDialogProps) => {
  ...
  const handleRestock = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `Restocked ${quantity} units of ${product.name}`,
      });
      onClose();
    } catch (error) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            ...
            {product.image && (
              <img src={product.image} alt={product.name} className="w-16 h-16 object-cover rounded mt-2" />
            )}
          </div>
